...
my_data[,1] # можн обратиться к первому стобцу. В таком случае мы увидим все значения первой переменной (столбца). 
...
# Комманда my_data$gender на выходе дает логический вектор из FALSE и TRUE
...
#### yt (rbind, cbind (rowbind - соединение по строкам, columnbind - соединение по столбцам)) _________________________________________________ ####
...
#### Step 6 of 11. Задача _____________________________________________________________________________________________________________________ ####
...
# то запиши в mtcars$even_gear значение 0, если нет то запиши значение 1
...
# то запиши в mtcars$even_gear2 значение 1
mtcars$even_gear2[mtcars$gear%%2 == 1] <- 0 # если остаток деления значения переменной mtcars$gear на 2 равно 1, 
# то запиши в mtcars$even_gear2 значение 0

# Вариант 4
data('mtcars')
a <- mtcars$gear # создадим переменную a и запишем в нее значения перменной mtcars$gear
even <- subset(mtcars, a%%2 == 0) # создадим дата фрейм even и сохраним в нем сабсет данных, значения переменной a <- mtcars$gear отвечает 
# условию: остаток деления а на 2 равен 0 (a%%2 == 0), т.е. четные значения
...
#### Step 8 of 11. Задача _____________________________________________________________________________________________________________________ ####
# Продолжим нашу работу с данными mtcars. Теперь ваша задача создать переменную - вектор mpg_4 и сохранить в нее значения 
# расхода топлива (mpg) для машин с четырьмя цилиндрами (cyl). 
...
#### Step 9 of 11. Задача _____________________________________________________________________________________________________________________ ####
...
#### Step 10 of 11. Задача ____________________________________________________________________________________________________________________ ####
# Укажите какая команда создаст сабсет данных mtcars, только для тех автомобилей, у которых число цилиндров (cyl) не равняется 3, 
# и время разгона автомобиля (qsec) больше среднего по выборке

new_data <- subset(mtcars, cyl == 3 & qsec < mean(qsec))
new_data <- subset(mtcars, cyl == 3 | qsec > mean(qsec))
new_data <- subset(mtcars, cyl != 3 | qsec < mean(qsec))
new_data <- subset(mtcars, cyl != 3 & qsec > mean(qsec)) # Правильный вариант



#### Step 11 of 11. Шпоры _____________________________________________________________________________________________________________________ ####
