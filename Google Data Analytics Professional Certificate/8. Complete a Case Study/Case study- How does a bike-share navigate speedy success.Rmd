...
library(janitor)
```
2. Have you ensured your data's integtity
Data integrity involves the accuracy, completeness, consistency, and trustworthiness of data throughout its lifecycle.
Yes. Data integrity was checked by:
* Accuracy: 
Data correctly represents the real-world values or events it's mean to describe.
No, some data in the "tripduration" column is not accuracy. `trips %>% filter(tripduration <= 0) %>% count()` rides end before it start and it's duration is shown as 0 or negative. 
```{r}
...
Data is factually correct and reflects the real world.
...
```{r eval=FALSE, label="Reading data"}
# Reading data
...
head(trip_2019)
lapply(list(trip_2019, trip_2020), head)
```



```{r eval=FALSE, label="colnames trip_2019"}
str(trip_2019)
str(trip_2020)
```
