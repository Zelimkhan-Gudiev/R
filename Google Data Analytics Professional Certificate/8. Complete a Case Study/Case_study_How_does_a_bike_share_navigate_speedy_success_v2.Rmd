---
title: 'Case study: How does a bike-share navigate speedy success_v2?'
author: "Zelimkhan"
date: '2025-05-31'
output:
...
however, the team needs to better understand how annual members and casual
...
|2| Consider key stakeholders | 1) Lily Moreno (Director of Marketing): Needs data-driven insights to justify marketing strategies. <br> 2) Cyclistic Executive Team: Requires compelling evidence to approve recommendations. <br> 3) Marketing Analytics Team: Relies on accurate analysis to guide campaign design. |
...
|№| Questions   | Answers                   |
|-|-------------|---------------------------|
|1| Where is your data located? | The data is publicly available in this two datasets: <br> *[Divvy 2019Q1](https://docs.google.com/spreadsheets/d/1uCTsHlZLm4L7-ueaSLwDg0ut3BP_V4mKDo2IMpaXrk4/template/preview?resourcekey=0-dQAUjAu2UUCsLEQQt20PDA#gid=1797029090) <br> *[Divvy 2020Q1](https://docs.google.com/spreadsheets/d/179QVLO_yu5BJEKFVZShsKag74ZaUYIF6FevLYzs3hRc/template/preview#gid=640449855) |
...
|3| Are there issues with bias or credibility in this data? Does your data ROCCC? | 1 ) Reliable: Yes, sourced from a licensed provider (Motivate International Inc.). <br> 2 ) Original: First-party system-generated (Cyclistic’s operational system) operational data. <br> 3 ) Comprehensive: Covers all trips during the specified quarters, including key fields. <br> 4 ) Current: Q1 2019 and Q1 2020 are accurate within their timeframes. <br> 5 ) Cited: Licensed and publicly cited via [Divvy's data agreement](https://divvybikes.com/data-license-agreement) <br> 6 ) Potential Bias: Excludes personally identifiable information (PII), so demographic analysis is limited. <br>|
|4| How are you addressing licensing, privacy, security, and accessibility? | 1 ) Licensing: Complies with Divvy's public data use license. <br> 2 ) Privacy: No percenal information concluded. <br> 3 ) Security: Data stored securely in google sheets. <br> 4 ) Accessibility: Data in google sheets format ensures compatibility with Excel, R or SQL.|
|5| How did you verify the data's integrity? | When reviewing the data in the tables, it was found that there is inconsistency in the name and number of columns between [Divvy 2019Q1](https://docs.google.com/spreadsheets/d/1uCTsHlZLm4L7-ueaSLwDg0ut3BP_V4mKDo2IMpaXrk4/template/preview?resourcekey=0-dQAUjAu2UUCsLEQQt20PDA#gid=1797029090) and [Divvy 2020Q1](https://docs.google.com/spreadsheets/d/179QVLO_yu5BJEKFVZShsKag74ZaUYIF6FevLYzs3hRc/template/preview#gid=640449855) datasets. <br> Data's integrity will be checked in more detail (accuracy, completness, consistency, trustworthiness) in the next step of analyses (step 3 "Process") |
|6| How does it help you answer you questions? | This data allows analysis of how casual riders and annual members use Cyclistic bikes differently by providing trip-level details such as ride length, frequency, timing, and bike type preferences. These insights are crucial for designing targeted marketing strategies.|
|7| Are there any problems with the data? | Yes: <br> 1 ) Column names are inconsistent between the two datasets (e.g., usertype in 2019 vs member_casual in 2020). <br> 2 ) The data does not include demographic or payment information, limiting deeper customer profiling. Seasonal and external factors are not directly captured but may affect usage. The dataset size requires careful handling to avoid memory issues. |

## 4.1. Key tasks
|№| Tasks       | Results |
|-|-------------|---------------------------------|
|1| Download data and store it appropriately| Downloaded [Divvy 2019Q1](https://docs.google.com/spreadsheets/d/1uCTsHlZLm4L7-ueaSLwDg0ut3BP_V4mKDo2IMpaXrk4/template/preview?resourcekey=0-dQAUjAu2UUCsLEQQt20PDA#gid=1797029090) and [Divvy 2020Q1](https://docs.google.com/spreadsheets/d/179QVLO_yu5BJEKFVZShsKag74ZaUYIF6FevLYzs3hRc/template/preview#gid=640449855) datasets. <br> Files stored in google drive folder [Source data](https://drive.google.com/drive/folders/1BO4tpPfs25t5z5Sq2a-neWuLGSlcVdyR). |
...
trip_2019 <- read_excel("C:/Users/GudievZK/Desktop/GitHub/Google Data Analytics Professional Certificate/8. Complete a Case Study/Divvy_Trips_2019_Q1.xlsx")
trip_2020 <- read_excel("C:/Users/GudievZK/Desktop/GitHub/Google Data Analytics Professional Certificate/8. Complete a Case Study/Divvy_Trips_2020_Q1.xlsx")
```

## 2. Data viewing <br>
### Looking at the tables <br>
...
head(trip_2020)
```
### Viewing the data structure <br>
Structure data about bike share in 2019
```{r label="Viewing the trip_2019 df structure"}
str(trip_2019)
```
Structure data about bike share in 2020
```{r label="Viewing the trip_2010 df structure"}
str(trip_2020)
```
...
|---|-----------------------------|------------------------------------|------------------------------------|
| 1 | Different number of columns | Has `r length(trip_2019)` columns  | Has `r length(trip_2020)` columns  |
| 2 | Different data              | Includes the following unique columns: <br> 1) bikeid <br> 2) tripduration <br> 3) gender <br> 4) birthyear  | Includes the following unique columns: <br> 1) rideable_type <br> 2) start_lat <br> 3) start_lng <br> 3) end_lat <br> 4) end_lng <br> |
| 4 | Columns with same data have different names | 1) trip_id <br>  2) start_time <br> 3) end_time <br>  4) usertype <br>  5) from_station_id <br> 6) from_station_name <br> 7) to_station_id <br> 8) to_station_name <br> | 1) ride_id <br> 2) started_at <br> 3) ended_at <br> 4) member_casual <br> 5)  start_station_id <br> 6) start_station_name <br> 7) end_station_id <br> 8) end_station_name  |
| 5 | Same values have different names | Column usertype has the following values: `r unique(trip_2019$usertype)` | Column member_casual has the following values: `r unique(trip_2020$member_casual)` |
