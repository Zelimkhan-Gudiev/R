...
for (i in 1:nrow(mydata)) {           # for принимает на вход вектор. Вектор от 1 до количества строк в mydata
  if (mydata$gender[i] == 'male') {   # if принимает на вход вектор и условие. Если i значние mydata$gender[i] равно 'male'
    print(mydata$score[i])            # print(mydata$score[i] выводит i значние переменной score. Выводит значние переменной score для всех строе где  значние mydata$gender[i] равно 'male' по всем строкам от 1 до nrow(mydata)
  }
}
...
yt_2$score <- ifelse((yt_2$numb_ret_depir >= 4 | yt$numb_ret_oiv >= 4), 'Bad', 'Norm') # отсупление
